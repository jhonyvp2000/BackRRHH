                ...
                <div className="container mx-auto max-w-7xl flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="hidden md:flex flex-col items-end">
                            <span className="font-bold text-sm">{session?.user?.name || "Dr. Juan Pérez"}</span>
                            <span className="text-xs opacity-80">{(session?.user as any)?.role || "Médico Internista"} - DNI {(session?.user as any)?.dni || "12345"}</span>
                        </div>
                        <div className="h-10 w-10 bg-white text-hospital-blue rounded-full flex items-center justify-center font-bold border-2 border-white/20">
                            {session?.user?.name?.substring(0, 2)?.toUpperCase() || "JP"}
                        </div>
                        <button
                            onClick={() => signOut({ callbackUrl: '/' })}
                            className="bg-red-500/20 hover:bg-red-500/40 p-2 rounded-lg transition-colors"
                            title="Cerrar Sesión"
                        >
            ...
            <div className="container mx-auto max-w-7xl px-6 py-8">
                    ...
                    <div className="lg:col-span-3">
                        <div className="bg-gradient-to-r from-blue-800 to-hospital-blue rounded-2xl p-8 text-white shadow-lg overflow-hidden relative">
                            <div className="relative z-10">
                                <h2 className="text-3xl font-bold mb-2">¡Buenos días! 👋</h2>
                                <p className="opacity-90 mb-6 max-w-lg">
                                    Tienes <strong>2 capacitaciones pendientes</strong> para este mes y tu rol de guardias ya está publicado.
                                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                    ...
                    <DashboardCard title="Gestión de RR.HH.">
                        <Link href="/seleccion/convocatorias">
